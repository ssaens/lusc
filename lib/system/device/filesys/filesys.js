...
import { luscshSrc } from '../../userlib/luscsh.js';
...
class Filesystem {
    ...
    _addDefPrograms(bin) {
        ...
        const luscsh = File.create(this.disk);
        const luscshInode = Inode.open(this.disk, luscsh);
        const luscshFile = new File(luscshInode);
        luscshFile.write(luscshSrc);
        luscshFile.close();

        binDir.add('luscsh.lsc', luscsh);
        binDir.close();
    }
}
